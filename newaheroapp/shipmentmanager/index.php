          ...
          <div class="col-lg-3 col-6">
            ...
            <div class="small-box bg-info">
              <div class="inner">
                      ...
                      <?php
                  $result=$mysqli->query("select *  from drivers ") or die(mysqli_error($mysqli));
               
                  while(list($driver_id,$driver_name,$email,$van_id)=mysqli_fetch_array($result))
                  {
                    $sql="SELECT COUNT(driver_id) AS total FROM drivers where status='pending'";
                    $result=$mysqli->query($sql);
                    $values=mysqli_fetch_assoc($result);
                    $num_rows=$values["total"];
                    echo "<h3>$num_rows</h3>";
              ...
              <div class="icon">
                ...
                <a href="" class="small-box-footer">View drivers <i class="fas fa-arrow-circle-right"></i></a>
            </div>
          </div>
          <!-- ./col -->
          <div class="col-lg-3 col-6">
            ...
            <div class="small-box bg-success">
              <div class="inner">
                <h3>  
                     <?php
                  $result=$mysqli->query("select *  from shipment ") or die(mysqli_error($mysqli));
               
                  while(list($product_id,$product_id,$customer_id,$oder_id,$driver_id)=mysqli_fetch_array($result))
                  {
                    $sql="SELECT COUNT(product_id) AS total FROM shipment where status='incomplete'";
                    $result=$mysqli->query($sql);
                    $values=mysqli_fetch_assoc($result);
                    $num_rows=$values["total"];
                    echo "<h3>$num_rows</h3>";
