          ...
          <div class="col-lg-3 col-6">
            ...
            <div class="small-box bg-warning">
              <div class="inner">
                      ...
                      <?php
                  $result=$mysqli->query("select *  from farms where status='approved' ") or die(mysqli_error($mysqli));
               
                  while(list($farm_id,$farm_name,$household_id,$household_name,$size,$status)=mysqli_fetch_array($result))
                  {
                    $sql="SELECT COUNT(farm_id) AS total FROM farms where status='approved'";
          ...
          <div class="col-lg-3 col-6">
            ...
            <div class="small-box bg-info">
              <div class="inner">
                  ...
                  $result=$mysqli->query("select *  from sales where status='pending' ") or die(mysqli_error($mysqli));
               
                  while(list($sale_id,$product_name,$quantity,$price,$oder_id,$status)=mysqli_fetch_array($result))
                  {
                    $sql="SELECT COUNT(sale_id) AS total FROM sales where status='approved'";
                    $result=$mysqli->query($sql);
                    $values=mysqli_fetch_assoc($result);
                    $num_rows=$values["total"];
                    echo "<h3>$num_rows</h3>";
          ...
          <div class="col-lg-3 col-6">
            ...
            <div class="small-box bg-danger">
              <div class="inner">
                  ...
                  $result=$mysqli->query("select *  from services") or die(mysqli_error($mysqli));
               
                  while(list($service_id,$service_name,$service_date,$start_time,$venue)=mysqli_fetch_array($result))
                  {
                    $sql="SELECT COUNT(service_id) AS total FROM services where status='approved'";
                    $result=$mysqli->query($sql);
                    $values=mysqli_fetch_assoc($result);
                    $num_rows=$values["total"];
                    echo "<h3>$num_rows</h3>";
