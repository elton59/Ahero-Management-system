          ...
          <!--card body-->
                  <div class="card-body">
                    ...
                    <table id="example2" class="table table-bordered table-hover" id="page1">
                      <thead>
                        <tr><th>PAYMENT_ID</th><th>PRODUCT_NAME</th><th>FARMER_NAME</th><th>TRANSCATION_ID</th><th>QUANTITY</th><th>STATUS</th><th>ACTION</th></tr>
                      </thead>
                    
                       <?php
                  $result=$mysqli->query("select * from fpay ")or die($mysqli->error);
                  while($row=$result->fetch_assoc())
                  {
                    ...
                    <td>".$row['amount']."</td>
                    <td>".$row['status']."</td>
                  <td> <a href='fpay.php?aid=$row[payment_id]' class='btn btn-success'>Recieved<a>
...
<?php
  ...
  {
    $payment_id=$_GET['aid'];
    $result = $mysqli->query("UPDATE fpay SET Status= 'recieved' WHERE payment_id = $payment_id") or die($mysqli->error);
